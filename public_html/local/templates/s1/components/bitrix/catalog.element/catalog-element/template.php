...
$type = $arResult['PROPERTIES']['TYPE']['VALUE'];
$composition = $arResult['PROPERTIES']['COMPOSITION']['VALUE'] ? $arResult['PROPERTIES']['COMPOSITION']['VALUE']['TEXT'] : "";
$delivery = $arResult['PROPERTIES']['DELIVERY']['VALUE'] ? $arResult['PROPERTIES']['DELIVERY']['VALUE']['TEXT'] : "";
$care = $arResult['PROPERTIES']['CARE']['VALUE'] ? $arResult['PROPERTIES']['CARE']['VALUE']['TEXT'] : "";
$equip = $arResult['PROPERTIES']['EQUIP']['VALUE'] ? $arResult['PROPERTIES']['EQUIP']['VALUE']['TEXT'] : "";
?>
    <div class="catalog-detail-top" id="<?= $itemIds['ID'] ?>"
            ...
            <div class="swiper-wrapper">
                <? foreach ($gallery as $index => $image): ?>
                    <div class="swiper-slide catalog-detail-gallery-card">
                        ...
                        <? if ($index === 0) : ?>
                            <div class="catalog-detail-gallery-card__label"
                                 id="<?= $itemIds['LABEL_ID'] ?>"<? if (!$label) : ?> style="display: none;"<? endif; ?>>
                                <?= $label ?>
                            </div>
                        <? endif; ?>
                        <a href="<?= $image['realSrc'] ?>" class="link-as-card"
        ...
        <div class="catalog-detail-card">
            <div class="container catalog-detail-card-container">
                <? if ($type): ?>
                    <div class="catalog-detail-card__type"><?= $type ?></div>
                <? endif; ?>
                <? if ($arParams['DISPLAY_NAME'] === 'Y'): ?>
                    <h1 class="catalog-detail-card__title"><?= $name ?></h1>
                <? endif; ?>
                <div class="catalog-detail-card-price">
                    <div class="catalog-detail-card-price-base">
                        <span class="catalog-detail-card-price__value"
                              id="<?= $itemIds['PRICE_ID'] ?>"><?= FormatNumberHelper::formatNumber($price['RATIO_PRICE']) ?></span><span
                                class="catalog-detail-card-price__currency">₽</span>
                    </div>
                    <? if ($arParams['SHOW_OLD_PRICE'] === 'Y'): ?>
                        <div class="catalog-detail-card-price-old"
                             style="display: <?= ($showDiscount ? '' : 'none') ?>;">
                            <span class="catalog-detail-card-price__value"
                                  id="<?= $itemIds['OLD_PRICE_ID'] ?>"><?= FormatNumberHelper::formatNumber($price['RATIO_BASE_PRICE']) ?></span><span
                                    class="catalog-detail-card-price__currency">₽</span>
                        </div>
                    <? endif; ?>
                </div>
                    ...
                    <div id="<?= $itemIds['TREE_ID'] ?>">
                        <? foreach ($arResult['SKU_PROPS'] as $skuProperty) : ?>
                            ...
                            <div class="catalog-detail-card-sku"
                                ...
                                <div class="catalog-detail-card-sku__title"><?= htmlspecialcharsEx($skuProperty['NAME']) ?></div>
                                <ul class="catalog-detail-card-sku-list">
                                    <? foreach ($skuProperty['VALUES'] as &$value) : ?>
                                        <? if (!$value['ID']) continue; ?>
                                        <? $value['NAME'] = htmlspecialcharsbx($value['NAME']); ?>
                                        <li class="catalog-detail-card-sku-item"
                                            title="<?= $value['NAME'] ?>"
                                            data-treevalue="<?= $propertyId ?>_<?= $value['ID'] ?>"
                                            data-onevalue="<?= $value['ID'] ?>">
                                            <span class="catalog-detail-card-sku-item__value"><?= $value['NAME'] ?></span>
                                            <? if ($value['NOTE']) : ?>
                                                <span class="catalog-detail-card-sku-item__note"><?= $value['NOTE'] ?></span>
                                            <? endif; ?>
                                        </li>
                                    <? endforeach; ?>
                                </ul>
                ...
                <div class="catalog-detail-card-stock catalog-detail-card-stock__available"
                     id="<?= $itemIds['AVAILABLE_MESS'] ?>"<? if (!$actualItem['CAN_BUY']) : ?> style="display: none;"<?endif; ?>>
                    <div class="catalog-detail-card-stock__icon"></div>
                    <div class="catalog-detail-card-stock__text">Доставим сегодня</div>
                </div>
                <div id="<?= $itemIds['NOT_AVAILABLE_MESS'] ?>"<? if ($actualItem['CAN_BUY']) : ?> style="display: none;"<?endif; ?>>
                    ...
                    <div id="<?= $itemIds['BASKET_ACTIONS_ID'] ?>"
                        ...
                        if ($showAddBtn) {
                            ...
                            <div class="catalog-detail-card-button-container">
                                <button class="button button_filled-pink catalog-detail-card__to-cart" type="button"
                        ...
                        if ($showBuyBtn) {
                            ?>
                            <div class="catalog-detail-card-button-container">
                                <button class="button button_filled-pink catalog-detail-card__to-cart"
                                        id="<?= $itemIds['BUY_LINK'] ?>" type="button">
                ...
                <? if ($composition) : ?>
                    <div class="catalog-detail-card-composition">
                        ...
                        <div class="catalog-detail-card-composition__value"><?= $composition ?></div>
                    </div>
                <? endif; ?>
                        ...
                        <a href="#delivery" class="catalog-detail-card-info-item__link" data-fancybox-slide>
                            ...
                            <span class="catalog-detail-card-info-item__icon">
                                <svg xmlns="http://www.w3.org/2000/svg" width="7" height="12" fill="none"
                                     viewBox="0 0 7 12"><path
                                            stroke="#242424" stroke-linecap="round" stroke-miterlimit="10"
                                            stroke-width=".5"
...
<? else: ?>
    <? App::Include('catalog/delivery') ?>
<? endif; ?>

<? if ($care) : ?>
    <div class="catalog-modal" id="care">
        <div class="catalog-modal-top">
            <div class="container catalog-modal-top-container">
                <div class="catalog-modal__title">Уход за букетом</div>
                <button class="catalog-modal__close" type="button" data-fancybox-close>
                    <svg xmlns="http://www.w3.org/2000/svg" width="27" height="26" fill="none" viewBox="0 0 27 26">
                        <path stroke="#242424" stroke-linecap="round" stroke-width=".7"
                              d="M3.707 4.04 23 23.335M23 3.743 3.707 23.036"/>
